# Step 31: smoke-config (외부 경계 — 시크릿·운영 마이그레이션·실호출 스모크)
...
### 3-1. 기존 송장 즉시 마이그레이션 확인

배포 후 cron webhook 등록 sweep(step5 B)이 기존 active 추적분을 등록한다. 운영 송장이 소수라 **첫 fire(≤15분)에 일괄 등록**된다. **true 즉시** 가 필요하면 배포 직후 cron 을 1회 수동 트리거(아니면 ≤15분 대기). 확인:
```bash
# 등록 가능 단계인데 아직 미등록(NULL)인 active 송장 수 → cron 후 0 으로 수렴해야 함
npx wrangler d1 execute unboxing --remote --command \
  "SELECT count(*) AS pending FROM shipments WHERE active=1 AND webhook_expires_at IS NULL AND last_normalized_status NOT IN ('미등록','배송완료')"
```
