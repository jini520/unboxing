...
### B. 폴링→webhook 승급 + 기존 송장 지연 마이그레이션

- `pollOne` 에서 **단계 전환 여부와 무관하게**, 폴링한 active 송장마다 `shouldRegisterWebhook(stage, active, webhook_expires_at, now)` 를 평가 → true 면 `registerTrackWebhook`(step3) 호출 → 성공 시 `webhook_expires_at` set. 이 **하나의 게이트**가 두 경우를 모두 덮는다:
  - **승급(W7)**: 미등록(이벤트 0)→첫 이벤트로 등록 가능 단계가 된 송장.
  - **지연 마이그레이션**: 이 전환 전부터 추적 중이던 **기존 `webhook_expires_at` NULL active 송장**(운영 D1 의 기존 추적분). 다음 폴백 폴링 때 등록 → ~12h 간격으로 낙하. 별도 백필 잡 없이 **정상 폴링 주기 내 webhook 으로 수렴**(ADR-028·ENGINEERING "backfill 불필요(지연 마이그레이션)").
- **자기 제한적·버스트 없음**: 기존 송장은 NULL 이라 `fallbackInterval` 이 평소 적응형 간격을 쓰므로 등록이 평소 cadence 에 분산되고(아래 E 예산 안), 등록되면 ~12h 로 떨어져 이후 폴링 부하가 줄어든다. 일괄 등록 burst 를 만들지 마라.
...
- 우선순위: **① 24h 재등록 sweep(신선도 핵심) → ② 폴백 폴링 → ③ receipt sweep 등**. 합계 ≤50 subrequest, 비동기 track 은 10 req/s 준수. **초과분은 다음 fire 로 이월**(W10). 재등록을 폴백보다 먼저 예산 배정.

### F. 테스트 (F-2·F-3)

재등록 sweep(임박 active 만·`now` 주입) / 조건부 폴백 due(webhook 송장 ~12h 전 due ❌·NULL 적응형) / lifecycle 독립 sweep(재폴링 0 webhook 송장도 비활성+알림·W11) / 슬롯(DELETE·active=0 → 재등록 제외) / 승급(미등록→첫 이벤트→등록·W7) / **지연 마이그레이션(기존 `webhook_expires_at` NULL·등록 가능 단계 active 송장이 단계 전환 없이도 다음 폴링서 등록·`expires_at` set → 이후 due 간격 ~12h)** / 예산(재등록 우선·이월·W10).
