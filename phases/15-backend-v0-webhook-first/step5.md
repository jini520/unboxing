...
- `pollOne` 에서 단계 전환을 처리한 뒤, `shouldRegisterWebhook(newStage, active, webhook_expires_at, now)` true 면 `registerTrackWebhook`(step3) 호출 → 성공 시 `webhook_expires_at` set. 미등록(이벤트 0)→첫 이벤트 송장이 여기서 webhook 으로 승급(W7 설계 경로).
...
재등록 sweep(임박 active 만·`now` 주입) / 조건부 폴백 due(webhook 송장 ~12h 전 due ❌·NULL 적응형) / lifecycle 독립 sweep(재폴링 0 webhook 송장도 비활성+알림·W11) / 슬롯(DELETE·active=0 → 재등록 제외) / 승급(미등록→첫 이벤트→등록·W7) / 예산(재등록 우선·이월·W10).
