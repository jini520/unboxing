 ...
 *
 * SQLi·로그 금지 정적 감사는 grep 으로 수행했고 결과는 docs/QA.md "정적 감사 결과" 표에 기록한다
...
describe("E2E 수명주기·삭제·개인정보 — 등록은 HTTP, 폴링은 주입 fetch/now", () => {
  ...
  it("등록 30일 경과(미완료·이동중) → active=0 + '분실 의심' 알림", async () => {
    ...
    const id = await registerShipment("dev-A", "123456789012");
    const f = makeFetch();
    f.status = "IN_TRANSIT"; // 이동중(첫 진입 알림 단계 — ADR-030) — 30일 경과로 강제 비활성 + 분실 알림

    // now 는 주간(KST) 고정 — '분실 의심'(비긴급 안내)이 step3 조용시간에 보류되지 않게(즉시 발송 단언 유지).
    // days=31 로 호출해 정오 스냅(최대 −12h)에도 age ≥ 30일을 보장(분실의심30일 트리거).
    await runPollingBatch(env, { now: daytimeKstDaysLater(Date.now(), 31), fetch: f.fetch });

    const r = await rowOf(id);
    expect(r?.active).toBe(0);
    // 발송 2건: ① 폴링 미등록→이동중 첫 진입 알림(ADR-030) ② 그 뒤 lifecycle sweep 의 '분실 의심'.
    // (폴링이 lifecycle sweep 보다 먼저라 sentMessages[0]=이동중, [1]=분실 의심.)
    expect(f.sendCalls).toBe(2);
    expect(f.sentMessages[1].body).toContain("오래 변동이 없어요");
    expect(f.sentMessages[1].data.shipment_id).toBe(id);
  });
  ...
  // ── 4. 발견 갭 (todo + docs/QA.md) ──

  it("QA-008/#11 수정: DELETE /me 는 push_tickets·notification_queue 의 잔존 push_token 도 즉시 폐기", async () => {
    await registerDevice("dev-A", TOKEN_A);
    await registerShipment("dev-A", "123456789012");
    const f = makeFetch();
    f.status = "INFORMATION_RECEIVED";
    // 주간 폴링: 등록 전환 즉시 발송 → push_tickets 1행(receipt 대기 버퍼에 token 사본).
    await runPollingBatch(env, { now: daytimeKstDaysLater(Date.now(), 0), fetch: f.fetch });
    expect(await count("SELECT COUNT(*) AS c FROM push_tickets")).toBe(1);
    // 야간 보류 큐에도 같은 token 사본이 있는 상황을 직접 적재(shipment_id NULL → 토큰 기반 폐기를 격리 검증).
    await env.DB.prepare(
      "INSERT INTO notification_queue (id, shipment_id, push_token, title, body, created_at) " +
        "VALUES ('q1', NULL, ?, 't', 'b', ?)",
    )
      .bind(TOKEN_A, Date.now())
      .run();
    expect(await count("SELECT COUNT(*) AS c FROM notification_queue")).toBe(1);

    expect((await call("DELETE", "/me", { deviceId: "dev-A" })).status).toBe(204);
    expect(await count("SELECT COUNT(*) AS c FROM devices")).toBe(0); // 정규 토큰 저장소 폐기
    // ADR-017 '푸시 토큰 폐기'는 즉시·완전 — 비-FK 사본(push_tickets·notification_queue)도 0건.
    expect(await count("SELECT COUNT(*) AS c FROM push_tickets")).toBe(0); // QA-008
    expect(await count("SELECT COUNT(*) AS c FROM notification_queue")).toBe(0); // #11
  });

  // QA-007 수정 완료 → 위 "미등록 7일 → … '번호 확인' 안내 1회" 테스트가 긍정 단언으로 검증한다(예외7일은 정책상 안내 생략).
  // QA-008/#11 수정 완료 → 위 테스트가 push_tickets·notification_queue 의 토큰 즉시 폐기를 긍정 단언으로 검증한다.

  // ── 정적 감사(grep) 결과 — docs/QA.md "정적 감사 결과" 표 (워커 런타임에서 소스 미열람) ──
