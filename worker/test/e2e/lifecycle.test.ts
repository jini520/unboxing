...
describe("E2E 수명주기·삭제·개인정보 — 등록은 HTTP, 폴링은 주입 fetch/now", () => {
  ...
  it("미등록 7일 → active=0 (무알림)", async () => {
    await registerDevice("dev-A", TOKEN_A);
    const id = await registerShipment("dev-A", "123456789012");
    const f = makeFetch();
    f.status = null; // 7일간 데이터 없음(미등록 유지)

    // 등록 직후 base → created_at 정합. now=base+7일 이면 lifecycle 이 미등록7일 비활성.
    await runPollingBatch(env, { now: Date.now() + 7 * DAY, fetch: f.fetch });

    expect((await rowOf(id))?.active).toBe(0);
    // QA-007: PRD 플로우6("7일 미수신 시 '번호 확인' 안내 후 비활성")이 기대하는 안내가 발송되지 않는다.
  ...
  it("예외 7일 → active=0 (전환 알림 1회 외 추가 안내 없음)", async () => {
    ...
    // 2) 7일 경과 — 동일 단계(무전환) + lifecycle 예외7일 비활성. 추가 '안내'는 없다(QA-007).
  ...
  // 사양(PRD 플로우6 · ARCH "데이터 수명주기" 표 "+안내"): 미등록7일·예외7일 비활성 시 '번호 확인' 안내 — 미구현(무발송).
  it.todo("QA-007: 미등록7일·예외7일 비활성 시 '번호 확인' 안내가 발송돼야 한다(현재 무발송)");
