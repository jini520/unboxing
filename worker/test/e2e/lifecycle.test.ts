...
describe("E2E 수명주기·삭제·개인정보 — 등록은 HTTP, 폴링은 주입 fetch/now", () => {
  ...
  it("QA-008 수정: DELETE /me 는 push_tickets 의 잔존 push_token 도 즉시 폐기", async () => {
    await registerDevice("dev-A", TOKEN_A);
    await registerShipment("dev-A", "123456789012");
    const f = makeFetch();
    f.status = "INFORMATION_RECEIVED";
    // 폴링: 등록 전환 즉시 발송(ADR-030) → push_tickets 1행(receipt 대기 버퍼에 token 사본).
    ...
    // ADR-017 '푸시 토큰 폐기'는 즉시·완전 — 비-FK 사본(push_tickets)도 0건.
