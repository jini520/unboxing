...
- ✅ **실 콜백 수신**(2026-06-27 확인): tracker.delivery 가 **실제로 콜백을 쏜다**(`wrangler tail` 로 `POST /webhooks/track` 관찰). ① 446 로젠(webhook분·`이동중`)에 02:38 콜백 도착 → 올바른 시크릿·D1 active 송장 통과 → track 재조회 → **같은 단계라 CAS no-op·중복 푸시 0**(멱등 라이브 확인). ② 스키마 스모크 잔여 webhook(가짜 번호·PLACEHOLDER 시크릿)의 콜백은 **시크릿 불일치 401**(방어 라이브 동작).
- ⏳ **남은 관찰**: 콜백→**실제 단계 전환→푸시**(다운스트림 `processShipmentUpdate` 는 폴링과 동일·검증됨 — 446 `배송출발` 시 자연 확인) · **서명(HMAC) 헤더 유무**(있으면 ADR-029 ① 서명검증 추가). 미수신/실패는 **폴백 폴링이 흡수**(머지 차단 아님).
