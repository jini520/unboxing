...
> **실행 결과(2026-06-26~27 운영 배포 후)** — 핵심 경계 green, **콜백 실수신까지 확인**. 상세 `docs/ENGINEERING.md` "스모크 확정 사실".
...
- ✅ **실 콜백 수신**(2026-06-27 확인): tracker.delivery 가 **실제로 콜백을 쏜다**(`wrangler tail` 로 `POST /webhooks/track` 관찰). ① 446 로젠(webhook분·`이동중`)에 02:38 콜백 도착 → 올바른 시크릿·D1 active 송장 통과 → track 재조회 → **같은 단계라 CAS no-op·중복 푸시 0**(멱등 라이브 확인). ② 스키마 스모크 잔여 webhook(가짜 번호·PLACEHOLDER 시크릿)의 콜백은 **시크릿 불일치 401**(방어 라이브 동작). ③ **콜백→전환→푸시 전체 경로 확인(09:35)**: 446 `이동중→배송출발` 전환을 콜백이 트리거(`last_polled_at`=09:35:11 비-그리드=`Date.now()`=콜백, 12h 폴백 미도래) → "🚚 오늘 도착 예정" 푸시 발송(양 기기). 이벤트(08:14)↔감지(09:35) ~1.3h 갭은 택배사→tracker 전파 지연. ④ **24h 재등록 sweep**으로 webhook 만료 연장 확인(06-28 01:25→23:45).
- ⏳ **남은 관찰(하나)**: **서명(HMAC) 헤더 유무**(ADR-029 ① — tracker 가 제공하면 서명검증 추가). 미수신/실패는 **폴백 폴링이 흡수**(머지 차단 아님).
