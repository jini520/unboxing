...
- **registerTrackWebhook 실호출**: 반환값 스키마·`expirationTime` 48h 수락·**최대 TTL**·**재등록이 중복 생성인지 갱신인지**(W9) 확인.
- **미등록(이벤트 0) 번호 등록 가부**(W7) — 안 받아주면 폴링 승급 경로가 정답임을 확인.
- **실 콜백 수신**: 실 송장 상태 변화 시 `/webhooks/track/<secret>` POST 도착·페이로드 형태(`{carrierId,trackingNumber}`)·**서명 헤더 유무**(있으면 ADR-029 ① 서명검증 추가).
- **위조 콜백 차단**: 잘못된 시크릿/임의 번호 콜백이 무시(202·무처리)됨.
- **deregister API 존재 여부**(슬롯 즉시 회수 가능한지).
- **fetch 바인딩(P-1)**: `registerTrackWebhook`도 `deps.fetch` 주입 경로 → `fetch.bind(globalThis)` 누락 시 `Illegal invocation`. 실호출로만 드러남(`docs/ENGINEERING.md` "webhook 구현 함정" T1).
