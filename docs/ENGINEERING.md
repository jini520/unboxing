...
- **콜백 happy-path 전체 확인(2026-06-27)**: tracker 가 실제로 `/webhooks/track` 콜백을 쏜다. ① 02:38 446 동일단계 콜백 → CAS no-op·**중복 푸시 0**(멱등). ② **09:35 446 `이동중→배송출발` 전환을 콜백이 트리거 → "🚚 오늘 도착 예정" 푸시 발송**(`last_polled_at`=09:35:11 비-그리드=`Date.now()`=콜백, 12h 폴백 미도래 ⇒ 폴링 아님). 이벤트(08:14)↔감지(09:35) ~1.3h 갭은 택배사→tracker 전파 지연. ③ **24h 재등록 sweep**으로 만료 연장 확인. ④ PLACEHOLDER 콜백 시크릿 401. **webhook-first 베팅(콜백이 온다·전환·푸시) 운영 성립.**
- **미해소(하나)**: 서명(HMAC) 헤더 유무(ADR-029 ① — tracker 제공 시 검증 추가). (재등록 멱등 W9 는 목록 API 부재로 API 검증 불가·`webhook_expires_at`-skip 방어 / deregister API 부재 확정·48h 만료 회수.)
