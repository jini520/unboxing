...
**v1.1.3 (2026-06-30 — UX 개선 묶음 · 패치 · 설계 작성·검토 중):** v1.1.2 머지·배포 후 사용자 요청 5건. **서버·D1 무변경**(클라이언트/빌드 설정만). 설계 SoT: `PRD`/`ADR-040~044`/`UI_GUIDE` "v1.1.3"/`ARCHITECTURE` "v1.1.3"/`ENGINEERING` P-11. 버전 정책 ADR-035(다음 패치 v1.1.3 — 요청 제목 "v1.1.1"은 라벨, app.json 은 이미 1.1.2).
...
- 구현: 확정 후 harness phase 정의. #2~#4 는 UI(시뮬/실기기 재캡처), #5 는 빌드 설정(리빌드 후 편집 메뉴 스모크), 순수 로직 변화 적음(`displayRecipientName` 류 신규 함수 없음 — 네비/UI 위주).
