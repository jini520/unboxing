...
| 단계 | 폴링 간격 |
|---|---|
| `미등록` | **시간대 윈도(KST·ADR-031)**: 06–18시 15분 · 18–21시 1h · 21–06시 재폴링 skip(첫 폴링만 1회 허용) |
| `등록`·`집화`·`이동중`·`기타` | 4h |
| `배송출발` | 1h |
| `배송완료` | 중단 |
| `예외` | 12h |

> 위는 **webhook 미등록분(`webhook_expires_at` NULL)** 의 폴백 간격이다. webhook 등록분은 단계 무관 ~12h 안전망(신선도는 콜백, ADR-028). `미등록`은 webhook 불가라 시간대 윈도로 첫 이벤트를 주간 촘촘히 감지(ADR-031).
